...
import { getAllProperties } from '~/store/properties/propertiesActions';
...
function ProductsView(props) {
    ...
    const getAllProduct = React.useCallback(()=> dispatch(getAllProperties()),[dispatch])
    const intl = useIntl();
    ...
    const propertiesList:any = useSelector(store => store['properties']['propertiesList'])

    const [isEmptyList, setIsEmpty] = useState(true);
    // const hasActiveFilters = shop.activeFilters.length > 0;
    // const currentFiltersCount = shop.currentFilters.length;
    const hasActiveFilters = false;
    const currentFiltersCount = 0;
                    ...
                    <React.Fragment>
                        <div className={viewOptionsClasses}>
                            <div className="view-options__body">
                                ...
                                <div className="view-options__legend">
                                    <FormattedMessage
                                        id="TEXT_SHOWING_PROPERTIES"
                                        values={{
                                            from:(productsList?.pageno > 1 ? ((productsList?.pageno -1) * propertiesList?.limit)+1 : 1),
                                            to: (productsList?.pageno * propertiesList?.limit) > propertiesList?.totalCount ? propertiesList?.totalCount : (productsList?.pageno * propertiesList?.limit),
                                            total: propertiesList?.totalCount,
                                        }}
                                    />
                                </div>
                        ...
                        >
                            ...
                            <div className="products-list__content">
                                {propertiesList?.data?.map((property) => (
                                    <div key={property.id} className="products-list__item">
                        ...
                        <div className="products-view__pagination">
                            <nav aria-label="Page navigation example">
                                <Pagination
                                    current={propertiesList?.pageno}
                                    total={Math.ceil(propertiesList?.totalCount/propertiesList?.limit)}
                                    siblings={2}
